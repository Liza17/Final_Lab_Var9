...
1.  **Функциональные:**
    *  Загрузка JSON логов ≥100,000 записей
    *  Расчет статистики за <5 секунд
    *  Генерация отчетов в 3 форматах (TXT, CSV, JSON)
    *  Поддержка фильтрации по всем параметрам

2. **Технические:**
   *Покрытие тестами ≥80%
   *Обработка ошибок для всех сценариев
   *Потребление памяти <500MB для больших файлов
